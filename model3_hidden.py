...
W_1 = weight_variable([len(feat_input[0]), hidden_one])
b_1 = bias_variable([hidden_one])
l_1 = tf.matmul(x, W_1) + b_1  # multiply by weights and add bias
h_1 = tf.nn.sigmoid(l_1)  # apply sigmoid

W_1b = weight_variable([hidden_one, len(vins_layer[0])])
b_1b = bias_variable([len(vins_layer[0])])
l_1b = tf.matmul(h_1, W_1b) + b_1b  # multiply by weights and add bias
v = tf.nn.sigmoid(l_1b)  # apply sigmoid

# calculate error
first_cost = tf.nn.sigmoid_cross_entropy_with_logits(l_1b, v_)  # cost function: sigmoid cross entropy with logits

# implement the output layer
W_2 = weight_variable([len(vins_layer[0]), hidden_two])
b_2 = bias_variable([hidden_two])
l_2 = tf.matmul(v, W_2) + b_2
h_2 = tf.nn.sigmoid(l_2)

W_2b = weight_variable([hidden_two, len(phon_output[0])])
b_2b = bias_variable([len(phon_output[0])])
l_2b = tf.matmul(h_2, W_2b) + b_2b
y = tf.nn.sigmoid(l_2b)

# training
cost = tf.nn.sigmoid_cross_entropy_with_logits(l_2b, y_)  # cost function: sigmoid cross entropy with logits
train_step = tf.train.GradientDescentOptimizer(0.05).minimize((cost + first_cost))  # sets learning rates
...
n_epochs = 1000000  # total number of words to present
